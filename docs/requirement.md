...
> 핵심 기능: 고객들이 식당에 방문할 날짜와 메뉴를 미리 선택하면 이벤트 플래너가 주문 메뉴, 할인 전 총주문 금액, 증정 메뉴, 혜택 내역, 총혜택 금액, 할인 후 예상 결제 금액, 12월 이벤트 배지 내용을 보여주는 기능
...
- [ ] 메뉴는 중복 없이 입력해야 한다. (메뉴 이름은 같지만 개수가 다른 경우도 포함)
- [ ] 메뉴판에 있는 메뉴만 입력해야 한다.
- [ ] 메뉴판에 없는 메뉴를 주문하면 메뉴판 안내 메시지가 출력된다.
- [ ] 위의 조건을 하나라도 만족하지 않으면 `[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.`가 출력되고, 다시 입력받는다.
- [x] 메뉴 개수가 20개를 초과할 경우 `메뉴는 한 번에 최대 20개까지만 주문할 수 있습니다.`가 출력된다.
- [x] 음료만 주문할 경우 `음료만 주문 시, 주문할 수 없습니다.`가 출력된다.
...
- [ ] `2023.12.1 ~ 2023.12.25`은 크리스마스 디데이 할인을 적용한다.
  - [ ] 1,000원으로 시작하여 크리스마스가 다가올수록 날마다 할인 금액이 100원씩 증가하도록 한다.
  - [x] 12월 1일에 1,000원, 2일에 1,100원, ..., 25일엔 3,400원 할인된다.
  - [ ] 총 주문 금액에서 해당 금액만큼 할인이 적용되도록 한다.
- [ ] 다만, `2023.12.26 ~ 2023.12.31`은 크리스마스 디데이 할인이 적용되지 않는다.

### 요일에 따라 메뉴당 할인 가격 계산 기능

- [ ] **평일**은 월요일~금요일이 아닌, **일요일~목요일**로 지정한다.
- [ ] 평일에는 디저트 메뉴를 메뉴 1개당 2,023원 할인되도록 한다.
- [ ] **주말**은 토요일/일요일이 아닌, **금요일/토요일**로 지정한다.
- [ ] 주말에는 메인 메뉴를 메뉴 1개당 2,023원 할인되도록 한다.

### 이벤트 달력에 따른 총주문 금액 특별 할인 가격 계산 기능

- [ ] 이벤트 달력에 별이 있는 날에는 총주문 금액에서 1,000원 특별 할인되도록 한다.
- [ ] 특별 할인 날짜는 `12/3(일), 12/10(일), 12/17(일), 12/24(일), 12/25(월), 12/31(일)`이다.

### 총 주문 금액에 따른 증정 이벤트 기능

- [ ] 할인 전 총주문 금액이 12만 원 이상일 때, 샴페인 1개가 증정되도록 한다.
- [ ] 총주문 금액 10,000원 이상부터 이벤트가 적용되도록 한다.
- [ ] 총주문 금액 10,000원 미만인 경우 `총주문 금액 10,000원 이상부터 이벤트가 적용됩니다.`가 출력된다.
- [ ] 증정 이벤트에 해당하지 않는 경우, 증정 메뉴 `없음`으로 보여 준다.

### 이벤트 기간에 따라 프로모션을 적용하는 기능

- [ ] '크리스마스 디데이 할인'을 제외한 다른 이벤트는 2023.12.1 ~ 2023.12.31 동안 적용되도록 한다.

### 혜택 내역을 출력하는 기능

- [ ] 해당 고객에게 적용된 이벤트 내역만 보여준다.
- [ ] 적용된 이벤트가 하나도 없다면 혜택 내역 `없음`으로 보여 준다.
- [ ] 혜택 내역에 여러 개의 이벤트가 적용된 경우는 아래와 같이 출력된다.
...
- [ ] `총혜택 금액 = 할인 금액의 합계 + 증정 메뉴의 가격`으로 계산된다.

### 할인 후 예상 결제 금액을 계산해 출력하는 기능

- [ ] `할인 후 예상 결제 금액 = 할인 전 총주문 금액 - 할인 금액`으로 계산된다.
- [ ] 이때, 증정 메뉴의 가격은 할인 후 예상 결제 금액에 포함하지 않는다.
...
- [ ] 적용된 이벤트가 하나도 없는 경우는 아래 예시와 같이 출력된다.
...
- [ ] 고객이 메뉴와 메뉴의 개수를 입력하면 이벤트 플래너가 출력된다.
- [ ] 아래와 같은 형식으로 이벤트 플래너가 출력된다.
...
- [ ] 에러 메시지 출력과 함께 해당 부분부터 입력을 다시 받는다.
- [ ] 주문이 정상 처리 되면, 이벤트 혜택 내역과 동시에 이벤트 플래너가 종료된다.

<br/>

## 🛠️ 객체별 명세

> MVC 패턴에 Layered Architecture 적용
...
- [ ] 프로그램 실행의 시작점인 `App.js`의 `run` 메서드로 실행시킬 수 있는가?
...
- [ ] 제공된 `InputView`, `OutputView` 객체를 활용해 구현했는가?
